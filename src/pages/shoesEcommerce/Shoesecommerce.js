  ...
  return (
    <div className={theme ? "porfolio dark" : "porfolio"}>
      <div className="aboutNavContainer">
        <nav className={open ? "contactNav active" : "contactNav "} style={{backgroundColor: "#C88E01"}}>
          <FontAwesomeIcon
            icon={faBars}
            className="bars"
            onClick={handleOpen}
      ...
      <main className="portfolioMain">
        <ul className={theme ? "imageSwitcher dark" : "imageSwitcher"}>
          <li onClick={switchImage} style={imgswitch ? {color: ""}: {color: "#C88E01"}}>{texts.desktop}</li>
          <li onClick={switchImage} style={imgswitch ? {color: "#C88E01"}: {color: ""}}>{texts.mobile}</li>
        </ul>
        <div className="porfolioImages">
          <img src={shoesDesktop} alt="" className="desktop" className={imgswitch ?"desktop hide": "desktop"}/>
          <img src={shoesMobile} alt="" className="mobile" className={imgswitch ?"mobile ": "mobile hide"} />
        </div>
      </main>
    </div>
  );
